...
def execute_query(query, params=(), fetch_one=False, fetch_all=False, fetch_lastrowid=False):
    try:
        with sqlite3.connect(app.config.get('DATABASE')) as conn:
            cursor = conn.cursor()
...
def delete_menu_item(item_id):
    # Ensure the logic for deleting the item and handling the database is correct
    query = "SELECT Image FROM Menu WHERE ID = ?"
    current_image = execute_query(query, (item_id,), fetch_one=True)
    current_image = current_image[0] if current_image else None
...
def add_menu_item():
    ...
    if request.method == 'POST':
        # Get menu item details from form data
        item_names = request.form.getlist('item_name[]')  # Get all item names
        prices = request.form.getlist('price[]')  # Get all prices
        descriptions = request.form.getlist('description[]')  # Get all descriptions

        # Get uploaded images
        image_files = request.files.getlist('image[]')  # Get the list of uploaded images

        for index, (item_name, price, description) in enumerate(zip(item_names, prices, descriptions)):
            data = {
                'Restaurant_id': restaurant_id,
                'Item_name': item_name,
                'Price': price,
                'Description': description,
                'Image': None  # Default to None if no image is uploaded
            }

            # Handle the image for this menu item
            if index < len(image_files):  # Ensure there's a corresponding image
...
def orders_management():
    ...
    if not restaurant_id:
        return redirect(url_for('restaurant_login'))  # Redirect to login if session is invalid

    orders_query = """
    SELECT Orders.Orders_id, Orders.Total_Amount, Orders.Remark, Orders.Order_Date, Orders.Order_Status,
           Customer.Name, Customer.Street, Customer.House_number, Customer.Postcode, Customer.City
    FROM Orders
...
def update_order_status():
    ...
    allowed_statuses = {'Pending', 'In Preparation', 'Completed', 'Cancelled'}
    if new_status not in allowed_statuses:
        return "Invalid status value", 400  # Bad request if status is not allowed
...
def customer_signin():
    if request.method == 'POST':
        ...
        if customer:
            ...
            return render_template('customer_signin.html', error="Invalid email or password")
...
def checkout():
    ...
    VALUES (?, ?, ?, ?,'Pending')
    """
    connection = sqlite3.connect(app.config.get('DATABASE', 'Main.db'))
